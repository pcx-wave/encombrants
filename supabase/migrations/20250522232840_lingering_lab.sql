/*
  ...
  1. Tables
    - users (with client, collector, deposit types)
    - collectors (for collector-specific data)
    - requests (for pickup requests)
    - availability_windows (for request time windows)
    - proposals (for collector proposals)
    - routes (for collection routes)
    - route_stops (for route waypoints)
    - disposal_sites (for waste disposal locations)

  2. Security
    - Enable RLS on all tables
    - Add appropriate policies for data access
    - Set up test accounts with proper credentials

  3. Test Data
    - Create test accounts for each user type
    - Set up a test collector profile
*/
...
VALUES 
  ('00000000-0000-0000-0000-000000000001', 'test.client@example.com', crypt('testclient123', gen_salt('bf')), now(), now()),
  ('00000000-0000-0000-0000-000000000002', 'test.collector@example.com', crypt('testcollector123', gen_salt('bf')), now(), now()),
  ('00000000-0000-0000-0000-000000000003', 'test.deposit@example.com', crypt('testdeposit123', gen_salt('bf')), now(), now())
